...
const HelloFakeShadow = ()=>{
    ...
    // 创建平行灯光
		...
		const shadowGeo = new PlaneGeometry(shadowSize, shadowSize);// 假阴影对应的几何平面

		const numberSphere = 15;// 将随机创建15个球体
		for(let i = 0; i < numberSphere; i++){
			const base = new Object3D(); // 创建球和阴影的整体对象，将小球和阴影都添加进去，让他们形成一个整体
			scene.add(base);
			...
			const shadowMesh = new Mesh(shadowGeo, shadowMat); // 通过创建网格让几何体和材质最终变成阴影物体网格
		...
		const render = (time: number)=>{
			...
			basesArray.forEach((item, index)=>{
				...
				base.position.set(Math.cos(angle)* radius, 0, Math.sin(angle)* radius); // 循环。改变position
				const yOff = Math.abs(Math.sin(time*2+index)); // 跳动。
